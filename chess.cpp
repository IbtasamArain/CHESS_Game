...
bool Rook::isValidMove(int toRow, int toCol, Board &board) const
{
    ...
    if (board.isFriendly(toRow, toCol, color))
        return false;
...
void Game::handlePawnPromotion(int row, int col)
{
    ...
    if ((sym == 'P' && row == 7) || (sym == 'p' && row == 0))
    {
        ...
        cout << "Tell to which piece your pawn should be promoted!";
        char p = ' ';
        cin >> p;
        while (p != 'q' && p != 'b' && p != 'r' && p != 'k')
        {
            cout << "Invalid choice!" << endl
                 << "Choose a legal piece" << endl;
            cin >> p;
        }
        if (p == 'q')
        {
        ...
        {
            board.setPiece(row, col, new Bishop(c, row, col)); // promote to Bishop
            cout << "  *** Pawn promoted to Bishop! ***\n";
        }
        if (p == 'r')
        {
            board.setPiece(row, col, new Rook(c, row, col)); // promote to Roook
            cout << "  *** Pawn promoted to Rook! ***\n";
        }
        if (p == 'k')
        {
            board.setPiece(row, col, new Knight(c, row, col)); // promote to Knight
            cout << "  *** Pawn promoted to Knight! ***\n";
        }
    }
}
