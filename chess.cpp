...
//  Includes: Piece, Pawn, Rook, Knight, Bishop, Queen, King, Board, Game
...
Piece::Piece(Color c, int r, int co) : color(c), row(r), col(co), hasMoved(false) {}
...
void Game::handlePawnPromotion(int row, int col) {
    ...
    if ((sym == 'P' && row == 7) || (sym == 'p' && row == 0)) {
        ...
        switch(choice){
            case 1:
            board.setPiece(row, col, new Queen(c, row, col)); 
            cout << "   Pawn promoted to Queen! \n";

            case 2:
            board.setPiece(row, col, new Rook(c, row, col)); 
            cout << "   Pawn promoted to Rook! \n";

            case 3:
            board.setPiece(row, col, new Bishop(c, row, col)); 
            cout << "   Pawn promoted to Bishoop! \n";
            
            case 4:
            board.setPiece(row, col, new Knight(c, row, col)); 
